...
module.exports = postcss.plugin('postcss-custom-pseudo-element', () => {
  ...
  const resolvePseudoSelector = (selector, warn) => {
    if (selector.indexOf(PSEUDO_PREFIX) > -1) {
      try {
        ...
        return selector.replace(pseudoSelector, ` [data-pseudo="${pseudoName}"]`);
      } catch (e) {
  ...
  return (root, result) => {
    // Replace pseudo selectors with their aliases
    root.walkRules(rule => {
      rule.selector = resolvePseudoSelector(rule.selector, err => rule.warn(result, err));
    });
  };
});
